...
from pyield import calendar as cd
...
def test_count_bdays_1():
    ...
    assert cd.count_bdays(start, end) == 5
...
def test_count_bdays_2():
    ...
    assert cd.count_bdays(start, end) == 10
...
def test_count_bdays_with_series():
    ...
    assert np.array_equal(cd.count_bdays(start, end), np.array([5, 15]))

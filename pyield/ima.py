...
def _process_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Converter a valores para inteiros
    df["MarketQuantity"] = (1000 * df["MarketQuantity"]).astype("Int64")
    df["MarketValue"] = (1000 * df["MarketValue"]).astype("Int64")
    df["QuantityVariation"] = (1000 * df["QuantityVariation"]).astype("Int64")

    return df


def ima(date: DateScalar) -> pd.DataFrame:
    """
    Fetch and process IMA market data for a given date.

    This function retrieves IMA market data for the specified date from ANBIMA,
